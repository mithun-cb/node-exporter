%define _rpmfilename %%{NAME}-%%{VERSION}-%%{RELEASE}.aarch64.rpm
...
Source0:        %{name}-%{version}.linux-arm64.tar.gz
...
%prep
%setup -q -n %{name}-%{version}.linux-arm64
...
%install
# Install the binary
install -D -m 0755 %{_builddir}/%{name}-%{version}.linux-arm64/node_exporter %{buildroot}/usr/local/bin/node_exporter
...
%changelog
* Sun Jun 07 2026 Your Name <mithuneeecu@gmail.com> - 1.11.1-1
- RPM build for node_exporter 1.11.1 (aarch64)
